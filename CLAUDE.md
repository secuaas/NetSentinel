# NetSentinel - Surveillance Réseau et Sécurité
...
NetSentinel est un outil de surveillance réseau et de détection d'intrusions pour les infrastructures SecuAAS.
...
- Intégration avec l'écosystème SecuAAS

## Stack Technique Prévu

- **Backend**: Go (pour la performance)
- **Capture réseau**: libpcap / gopacket
- **Base de données**: PostgreSQL + TimescaleDB (séries temporelles)
- **API**: REST + WebSocket pour temps réel
- **Frontend**: React avec dashboard temps réel

## Fonctionnalités Prévues

1. **Capture et analyse de paquets**
   - Inspection DPI (Deep Packet Inspection)
   - Analyse de protocoles (HTTP, DNS, TLS, etc.)

2. **Détection d'intrusions**
   - Règles Suricata/Snort compatibles
   - Détection par signatures
   - Détection par anomalies (ML)

3. **Monitoring**
   - Bande passante par hôte/service
   - Connexions actives
   - Latence et pertes
...
Projet en phase de conception initiale.
...
│   ├── analyzer/     # Analyse de trafic
│   ├── detector/     # Détection d'intrusions
